	...
	unsigned long event, void *ptr)
{
	...
	if (!mdwc->usb_data_enabled) {
		dev_dbg(mdwc->dev, "ignoring id notification, usb_data_enabled == false\n");
		return NOTIFY_DONE;
	}

	id = event ? DWC3_ID_GROUND : DWC3_ID_FLOAT;
	if(force_id != 2){
		dev_dbg(mdwc->dev, "id %d for event %;d, forcing $d instead\n", id, event, force_id);
		id = force_id;
	}
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	// extcon doesn't quite work without dash and with mata's pd stack
	// I wish dash charging never happened
	if(force_id != 2 && mdwc->id_state != force_id){
		dev_dbg(mdwc->dev, "init with id %d, forcing %d instead\n", mdwc->id_state, force_id);
		mdwc->id_state = force_id;
		dwc3_ext_event_notify(mdwc);
	}
