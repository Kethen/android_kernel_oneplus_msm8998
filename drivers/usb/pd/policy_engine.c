...
static bool disable_usb_pd = true;
		...
		size_t num_data, enum pd_sop_type sop)
{
	...
	if (ret < 0){
		usbpd_err(&pd->dev, "%s: pd_phy_write returned %d\n", __func__, ret);
		return ret;
	}else if (ret != num_data * sizeof(u32)){
		usbpd_err(&pd->dev, "%s: pd_phy_write expected write size: %d, actual write size: %d\n", __func__, num_data * sizeof(u32), ret);
		return -EIO;
	}
